...
pub fn connect() -> Result<Database, Error> {
    ...
    for migration in MIGRATE_V1.split(";").map(str::trim).filter(|s| !s.is_empty()) {
        conn.execute(&migration, NO_PARAMS)?;
    }
    Ok(Database {
        conn
    })
...
impl Database {
    ...
    pub fn get_last_pub_date(
        &self
    ) -> Result<NaiveDateTime, Error> {
        self.conn.query_row(
            "SELECT last_pub_date FROM tosho",
            NO_PARAMS,
            |row| row.get(0))
            .map_err(|e: RusqliteError| Error::DbError(e))
    }

    pub fn set_last_pub_date(
        &self,
    ...
    pub fn add_show_and_episodes(
        ...
        let show_id: i64 = trans.query_row(
            r#"SELECT MAX(show_id) + 1 FROM shows"#,
            NO_PARAMS,
            |row| row.get(0))?;
        trans.execute(
            r#"INSERT INTO shows
    ...
    pub fn add_episodes(
        &mut self,
        episodes: &Vec<(i64, i32, i32, String)>
    ) -> Result<(), Error> {
        let trans = self.conn.transaction()?;
